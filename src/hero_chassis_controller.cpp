...
{
  double error = command_ - front_left_joint_.getVelocity();

  // Set the PID error and compute the PID command with nonuniform time
  // step size. The derivative error is computed from the change in the error
  // and the timestep dt.
  double front_left_joint_commanded_effort = front_left_joint_pid_controller_.computeCommand(error, period);

  front_left_joint_.setCommand(front_left_joint_commanded_effort);
  ...
  if (loop_count_ % 10 == 0)
  {
    if (controller_state_publisher_ && controller_state_publisher_->trylock())
    {
      ...
      controller_state_publisher_->msg_.command = front_left_joint_commanded_effort;
